...
//		listeCours = coursDAO.findAll() ;
	...
	public String updateProfesseurGet(@PathVariable String code, Model model) {
		...
		model.addAttribute("savedId", professeur.getCode());
	...
	public String addUpdateProfesseurPost(@Valid Professeur professeur, BindingResult errors,
		...
		logger.debug("Professeur Info:" + professeur.getCode() + " savedId " + savedId);
		...
		if (errors.hasErrors()) {
			...
			logger.debug("Erreurs dans les données de professeur:" + professeur.getCode());
			return "professeur/addProfesseur";
		...
		if (savedId == null) {
			...
			if (professeurDAO.exists(professeur.getCode() )) {
				logger.debug("Le professeur existe:" + professeur.getCode() + " savedId " + savedId);
...
//			professeur.setCode( professeur.getCode() ) ;	
			...
			if (!savedId.equals( professeur.getCode() )) {
				// code à changé
				// Vérifie si pas en doublon avec un autre
				if (professeurDAO.exists(professeur.getCode())) {
					logger.debug("Le Professeur Existe:" + professeur.getCode() + " savedId " + savedId);
					throw new DuplicateException("Le Professeur " + professeur.getCode() + " existe déjà");
				}
				// retire le professeur avec l'ancien code
				professeurDAO.delete( savedId );
			} else {
				// retire le cours de la liste
				professeurDAO.delete( professeur.getCode());
			}
		...
		String adr = "redirect:/professeur/"   + professeur.getCode();
